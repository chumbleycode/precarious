...
# The mediators are either continuous (weight, socint) or ordered factors (soccap, EFFICACY2) with many levels
...
# # dat %>% select(EFFICACY2,socint, soccap, weight, HEALTH2, PRECARIAT9) %>% cor(method = "kendall")
# EFFICACY2      socint      soccap      weight     HEALTH2  PRECARIAT9
# EFFICACY2   1.00000000  0.17886851  0.21445834  0.03351416  0.24130723 -0.18811450
# socint      0.17886851  1.00000000  0.14477898  0.05859866  0.17514726 -0.15326912
# soccap      0.21445834  0.14477898  1.00000000 -0.01430031  0.11583341 -0.13221387
# weight      0.03351416  0.05859866 -0.01430031  1.00000000 -0.06430507 -0.02468539
# HEALTH2     0.24130723  0.17514726  0.11583341 -0.06430507  1.00000000 -0.14853815
# PRECARIAT9 -0.18811450 -0.15326912 -0.13221387 -0.02468539 -0.14853815  1.00000000
...
# Dubious ("negative"?) predictor of weight. Weight is not gaussian. What are
# the units? (what is weight < 2, which describes most people).
dat %>% gf_violin(weight~ factor(PRECARIAT9))
with(dat, anova(lm( weight ~ PRECARIAT9))) # continuous
...
outcome_equ = str_c(
  ...
  str_c("coef_", 1:7, "*", c("age", "female", "native", "weight", "poor","daded", "educ"), " + ", collapse = " " ),
...
fit <- sem(model = multipleMediation, data = dat, test = "bollen.stine" se = "bootstrap") # a way to robustify
fit <- sem(model = multipleMediation, data = dat, estimator = "MLM", se = "robust") # gets satora-bentley and fineses non-normality. https://www.youtube.com/watch?v=HvYW_GeHpD8
